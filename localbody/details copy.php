...
<html lang="en">
  ...
  <script>
    ...
    window.location.href = "index.php"; // Redirects to the new file/page
      ...
      if (!fromDate) {
        $('#from_date_error').text('Please enter a valid date.').show(); // Show error if date is empty
        return false;
      } else {
      ...
      // Basic validation to check if the input is empty
      if (!toDate) {
        $('#to_date_error').text('Please enter a valid date.').show(); // Show error if date is empty
        return false;
      }
      else {
        // Validate if the date is in the correct format (DD/MM/YYYY)
...
//debugger;
  ...
  <body>
    ...
    <input type="button" class="btn btn-primary" name="save-nomination" value="Back" id="redirectButton">
